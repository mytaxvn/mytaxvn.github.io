...
type Html with
    static member label(text: string) = Html.label [ prop.text text ]

module List =
    let changeAt i map list = list |> List.updateAt i (map list[i])
...
let roundInt64 (x: float) = x |> Math.Round |> int64
...
let formatNumber value = formatter.format value
